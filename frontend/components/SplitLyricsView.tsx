...
export interface SplitLyricsViewRef {
  addSplit: () => void;
  removeSplit: () => void;
  getPaneCount: () => number;
}
...
const SplitLyricsView = forwardRef<SplitLyricsViewRef, SplitLyricsViewProps>(
  function SplitLyricsViewComponent(props, ref) {
    const {
      lyrics,
      zoomLevel,
      textAlign = 'center',
      showSplitterControls = true,
    ...
    const addSplit = useCallback(() => {
      setPanes(currentPanes => {
        ...
        const newPanes = currentPanes.map(p => ({ ...p, heightPercent: equalHeight }));
        newPanes.push({ id: Date.now().toString(), heightPercent: equalHeight });
        return newPanes;
      });
    }, []);

    // Remove last split (can go down to 1 pane)
    const removeLastSplit = useCallback(() => {
      setPanes(currentPanes => {
        if (currentPanes.length <= 1) return currentPanes;
        const index = currentPanes.length - 1;
        const removedHeight = currentPanes[index].heightPercent;
        const remainingPanes = currentPanes.filter((_, i) => i !== index);
        const distributeHeight = removedHeight / remainingPanes.length;
        return remainingPanes.map(p => ({
          ...p,
    ...
    // Remove a specific split by index
    ...
    useImperativeHandle(ref, () => ({
      addSplit,
      removeSplit: removeLastSplit,
      getPaneCount: () => panes.length
    }), [addSplit, removeLastSplit, panes.length]);
    ...
    const renderLyrics = () => {
      ...
      // Split lyrics by double newlines to identify paragraphs
      const paragraphs = lyrics.split('\n\n');

      return (
        <div className="w-full max-w-4xl md:max-w-5xl lg:max-w-6xl xl:max-w-7xl mx-auto">
          <div
            className={`${alignClass} w-full text-white`}
            style={{
              fontSize,
              fontFamily,
          ...
          >
            {paragraphs.map((paragraph, index) => (
              <p
                key={index}
                style={{
                  marginBottom: index < paragraphs.length - 1 ? `${paragraphSpacing}em` : '0',
                  whiteSpace: 'pre-wrap'
                }}
              >
                {paragraph}
    ...
    return (
      <div ref={containerRef} className="h-full w-full flex flex-col relative">
        {/* Add/Remove Split Controls - Auto-hide on inactivity */}
        <div
          className={`
            absolute top-4 left-4 z-50 flex items-center gap-2
            transition-opacity duration-300
            ${showSplitterControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}
          `}
        >
          <button
            onClick={addSplit}
...
SplitLyricsView.displayName = 'SplitLyricsView';

export default SplitLyricsView;
